const minLength = min => value =>
  value && value.length < min ? `Minst ${min} tecken` : undefined;
const maxValue = max => value =>
  value && value > max ? 'För högt årtal' : undefined;
const minValue = min => value =>
  value && value < min ? 'För lågt årtal' : undefined;
const getCurrentYear = () => new Date().getFullYear();

export const numeric = value =>
  value && isNaN(Number(value)) ? 'Endast siffor' : undefined;
